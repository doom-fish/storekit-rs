...
- Excluded legacy `SK*` StoreKit 1 Objective-C APIs (StoreKit.framework/Headers/*.h) plus protocol-conformance/debug noise (`Equatable`, `Hashable`, `Sendable`, `debugDescription`, etc.).
...
- v2 audit re-verified against macOS 26.2.sdk; all key symbols from v1 confirmed present; no new macOS symbols or gaps detected since v1.
